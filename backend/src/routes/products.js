...
router.get(`/`, async (req, res) => {
  ...
  !productList
    ? res.status(500).send({ message: "No hay productos guardados." })
...
router.get("/:id", async (req, res) => {
  ...
  !product
    ? res.status(500).send({ message: "No existe un producto con este id" })
...
router.put("/:id", async (req, res) => {
  ...
  !product
    ? res.status(404).send({ message: "El producto no se pudo actualizar" })
    : res.status(201).send({ message: "Producto actualizado", product });
});
